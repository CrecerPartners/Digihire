...
export default function ProfileSetup({ profile, onUpdate }: Props) {
  ...
  const applyParsedCv = useCallback((parsed: Awaited<ReturnType<typeof parseCvWithOpenAI>>) => {
    setFormData(prev => ({
      ...prev,
  ...
  useEffect(() => {
    ...
    if (pendingLinkedin) {
      setFormData(prev => prev.linkedin_url ? prev : { ...prev, linkedin_url: pendingLinkedin });
      sessionStorage.removeItem(LINKEDIN_SESSION_KEY);
    }

    // Auto-trigger CV parse — no button click required
    const cvName = sessionStorage.getItem(CV_NAME_SESSION_KEY);
    const cvBase64 = sessionStorage.getItem(CV_SESSION_KEY);
    if (cvName && cvBase64 && !autoFillStarted.current) {
  ...
  return (
    <div className="rounded-xl bg-white shadow-sm border border-gray-200 overflow-hidden">
      ...
      {/* CV auto-fill status */}
      {(cvParsing || pendingCvName) && (
        <div className="mx-0 px-6 py-3 bg-sky-50 border-b border-sky-100 flex items-center gap-3">
          <Loader2 size={16} className="text-sky-500 shrink-0 animate-spin" />
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-sky-800">Analyzing your CV…</p>
            <p className="text-[11px] text-sky-600 truncate">{pendingCvName} — filling in your profile automatically</p>
          </div>
        </div>
      )}
      {cvError && (
        <div className="mx-0 px-6 py-3 bg-red-50 border-b border-red-100 flex items-center gap-3">
          <X size={16} className="text-red-500 shrink-0" />
          <p className="text-xs text-red-700 flex-1">CV auto-fill failed: {cvError}. You can fill in the fields manually.</p>
        </div>
      )}
