...
import {
  Calculator as CalculatorIcon, Plus, Trash2, Sparkles, Share2,
...
const Calculator = () => {
  ...
  const handleSaveTarget = () => {
    updateProfile.mutate({
      ...
      onError: (err: unknown) => toast.error(err instanceof Error ? err.message : "Failed to save target"),
    });
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Tabs value={mode} onValueChange={(v: string) => setMode(v as "quick" | "target")} className="w-fit">
          <TabsList className="bg-secondary/50 border border-border">
            ...
            <TabsTrigger value="quick" className="data-[state=active]:bg-background data-[state=active]:text-primary">
              <CalculatorIcon className="h-3.5 w-3.5 mr-1.5" /> Quick Est.
            </TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={timeframe} onValueChange={(v: string) => setTimeframe(v as "weekly" | "monthly")}>
                  ...
                  <SelectContent>
                    {products.map((p) => (
                      <SelectItem key={p.id} value={p.id}>
                        {p.name} — {formatNaira(p.price * (p.commissionRate / 100))} comm.
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <div className="flex items-center gap-1 w-full sm:w-auto">
                  <Button type="button" variant="outline" size="icon" className="h-9 w-9 shrink-0" onClick={() => setQuantity(q => Math.max(1, q - 1))}>
                    <Minus className="h-3.5 w-3.5" />
                  </Button>
                  <Input type="number" min={1} value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value) || 1)} className="w-16 text-center" />
                  <Button type="button" variant="outline" size="icon" className="h-9 w-9 shrink-0" onClick={() => setQuantity(q => q + 1)}>
                    <Plus className="h-3.5 w-3.5" />
                  </Button>
                </div>
                <Button onClick={addItem} className="volt-gradient">Add to Plan</Button>
              </div>
            </CardContent>
          </Card>
